...
#else
__RCSID("$Id: fasl.c,v 1.11 2014/12/25 23:57:55 christos Exp $");
#endif
#endif
...
/*
 * fasl  -  fast loader				j.k.foderaro this loader is
 * tuned for the lisp fast loading application any changes in the system
 * loading procedure will require changes to this file
 * 
 * The format of the object file we read as input: text segment: 1) program text
 * - this comes first. 2) binder table - one word entries, see struct bindage
 * begins with symbol:  bind_org 3) litterals - exploded lisp objects. begins
 * with symbol:  lit_org ends with symbol:    lit_end data segment: not used
 * 
 * 
 * these segments are created permanently in memory: code segment - contains
 * machine codes to evaluate lisp functions. linker segment - a list of
 * pointers to lispvals.  This allows the compiled code to reference constant
 * lisp objects. The first word of the linker segment is a gc link pointer
 * and does not point to a literal.  The symbol binder is assumed to point to
 * the second longword in this segment.  The last word in the table is -1 as
 * a sentinal to the gc marker. The number of real entries in the linker
 * segment is given as the value of the linker_size symbol.  Taking into
 * account the 2 words required for the gc, there are 4*linker_size + 8 bytes
 * in this segment. transfer segment - this is a transfer table block.  It is
 * used to allow compiled code to call other functions quickly.  The number
 * of entries in the transfer table is given as the value of the trans_size
 * symbol.
 * 
 * the following segments are set up in memory temporarily then flushed binder
 * segment -  a list of struct bindage entries.  They describe what to do
 * with the literals read from the literal table.  The binder segment begins
 * in the file following the bindorg symbol. literal segment - a list of
 * characters which _Lread will read to create the lisp objects.  The order
 * of the literals is: linker literals - used to fill the linker segment.
 * transfer table literals - used to fill the transfer segment binder
 * literals - these include names of functions to bind interspersed with
 * forms to evaluate. The meanings of the binder literals is given by the
 * values in the binder segment. string segment - this is the string table
 * from the file.  We have to allocate space for it in core to speed up
 * symbol referencing.
 * 
 */
...
extern intptr_t mcnts[], mcntp, doprof;
...
/*
 * mini symbol table, contains the only external symbols compiled code is
 * allowed to reference
 */


struct ssym {
	char           *fnam;	/* pointer to string containing name */
...
= {
	...
	"mcount", (intptr_t) mcount, -1,
	"mcnts", (intptr_t) mcnts, -1,
	"wnaerr", (intptr_t) wnaerr, -1,
	"qnewint", (intptr_t) qnewint, -1,
	"qnewdoub", (intptr_t) qnewdoub, -1,
	"qcons", (intptr_t) qcons, -1,
	"qoneplus", (intptr_t) qoneplus, -1,
	"qoneminus", (intptr_t) qoneminus, -1,
	"typetable", (intptr_t) typetable, -1,
	"tynames", (intptr_t) tynames, -1,
	"qget", (intptr_t) qget, -1,
	"errp", (intptr_t) &errp, -1,
	"Inonlocalgo", (intptr_t) Inonlocalgo, -1,
	"_erthrow", (intptr_t) _erthrow, -1,
	"error", (intptr_t) error, -1,
	"qpushframe", (intptr_t) qpushframe, -1,
	"retval", (intptr_t) &retval, -1,
	"lispretval", (intptr_t) &lispretval, -1,
#ifndef NPINREG
	"np", (intptr_t) &np, -1,
	"lbot", (intptr_t) &lbot, -1,
#endif
#ifndef NILIS0
	"nilatom", (intptr_t) &nilatom, -1,
#endif
	"bnp", (intptr_t) &bnp, -1,
	"Ibindvars", (intptr_t) Ibindvars, -1,
	"Iunbindvars", (intptr_t) Iunbindvars, -1
};

#define SYMMAX ((sizeof Symbtb) / (sizeof (struct ssym)))

#define roundup(x) (char *)(((intptr_t)x + 3) & ~3)	/* round to longword boundary */

struct nlist    syml;		/* to read a.out symb tab */
extern int     *bind_lists;	/* gc binding lists 	  */

/*
 * bindage structure: the bindage structure describes the linkages of
 * functions and name, and tells which functions should be evaluated.  It is
 * mainly used for the non-fasl'ing of files, we only use one of the fields
 * in fasl
 */
struct bindage {
...
/*
 * the possible values of b_type -1 - this is the end of the bindage entries
 * 0  - this is a lambda function 1  - this is a nlambda function 2  - this
 * is a macro function 99 - evaluate the string
 * 
 */
...
lispval Lfasl(void)
{
	...
	int             i, j, times;
	intptr_t	*iptr;
	int             funloc[MAXFNS];	/* addresses of functions rel to txt
					 * org */
	int             funcnt = 0;
	...
	/*
	 * symbols which hold the locations of the segments in core and in
	 * the file
	 */
...
#ifdef notdef
	int             string_file_org;	/* location of string table
						 * in file */
#endif
	int             string_size;	/* number of chars in string table */
	int             segsiz;	/* size of permanent incore segment */
	...
	filnm = verify(lbot->val, "fasl: non atom arg");
	...
	/*
	 * insure that the given file name ends in .o if it doesnt, copy to a
	 * new buffer and add a .o but Allow non .o file names (5mar80 jkf)
	 */
	tempfilbf[0] = '\0';
	...
	if (read(fileno(filp), string_core_org, string_size - 4)
	    != string_size - 4)
		error("fasl: string table read error ", FALSE);
	...
	{
		int             compar();
		qsort(funloc, funcnt, sizeof(intptr_t), compar);
	}
#endif
	...
	segsiz = bind_org + 4 * linker_size + 8 + 3;	/* size is core segment
							 * size plus linker
							 * table size plus 2 for
							 * gc list plus 3 to
							 * round up to word */
	 ...
	 * entries given by linker_size .  There must be a gc word at the
	 * beginning and a -1 at the end
	 */
	...
	if (debug)
		printf("lin_cor_org: %x, link_cor_end %x\n",
		       lc_org, lc_end);
#endif
	Symbtb[1].floc = (intptr_t) (lc_org + 4);

	/* set the linker table to all -1's so we can put in the gc table */
	for (iptr = (intptr_t *) (lc_org + 4);
	     iptr <= (intptr_t *) (lc_end);
	     iptr++)
		*iptr = -1;
	...
	if (Vpurcopylits->a.clb == nil) {
		*(int *) lc_org = (intptr_t) bind_lists;	/* point to current */
		bind_lists = (int *) (lc_org + 4);	/* point to first item */
	}
	...
	/*
	 * check if there is a transfer table required for this file, and if
	 * so allocate one of the necessary size
	 */

	if (trans_size > 0) {
		tranloc = gettran(trans_size);
		Symbtb[0].floc = (intptr_t) tranloc;
	}
	...
	/*
	 * the only symbols we will relocate are references to  external
	 * symbols.  They are recognized by extern and pcrel set.
	 */
...
#define offset(p) (((p).r_pcrel) ? ((intptr_t) code_core_org): 0)
	...
	/*
	 * the first forms we wish to read are those literals in the literal
	 * table, that is those forms referenced by an offset from r8 in
	 * compiled code
	 */
	...
	/*
	 * now process the binder table, which contains pointers to functions
	 * to link in and forms to evaluate.
	 */
	funcnt = 0;
	...
	for (; curbind->b_type != -1; curbind++) {
		...
		if (curbind->b_type <= 2) {	/* if function type */
			...
			if (uctolc)
				ouctolc = TRUE;	/* if changed by eval,
						 * remember */
			curibase = ibase->a.clb;
...
/*
 * clrtt :: clear transfer tables, or link them all up; this has two totally
 * opposite functions: 1) all transfer tables are reset so that all function
 * calls will go through qlinker 2) as many transfer tables are set up to
 * point to bcd functions as possible
 */
void
clrtt(int flag)
{
	/*
	 * flag = 0 :: set to qlinker flag = 1 :: set to function bcd binding
	 * if possible
	 */
	struct trtab   *temptt;
	struct trent   *tement;
	...
	for (temptt = trhead; temptt != 0; temptt = temptt->nxtt) {
		for (tement = &temptt->trentrs[0]; tement->fcn != 0; tement++) {
			if (flag == 0 || TYPE(fnb = tement->name->a.fnbnd) != BCD
				...
				tement->fcn = fnb->bcd.start;
		}
	}
}

/*
 * chktt - builds a list of transfer table entries which don't yet have a
 * function associated with them, i.e if this transfer table entry were used,
 * an undefined function error would result
 */
lispval
chktt(void)
{
...
void
add_offset(int *addr, int relocoffset)
{
	*addr += relocoffset;
}
