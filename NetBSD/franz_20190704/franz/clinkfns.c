...
#include <sys/cdefs.h>
#ifndef lint
__RCSID("$Id: clinkfns.c,v 1.1 2014/12/11 17:12:17 christos Exp $");
#endif
...
extern intptr_t *bind_lists;	/* gc binding lists */
extern int      uctolc;

static lispval *make_linktable(char *[], int);
static lispval  rdstr(char *);
...
)
{
	...
	lispval         currtab,/* current readtable and ibase -- must be */
	                curibase;	/* kept up to date as we evaluate
					 * forms */
	int             ouctolc;/* ditto uctolc flag */
	int             note_redef = clink_note_redef;	/* remember in case
							 * reset */
	...
	/*
	 * To read in the forms correctly, the readtable &c must be the same
	 * as when they were printed.  However, we must also remember if
	 * these things change when we evaluate a form so that the changes
	 * will be in force for subsequent evaluations.
	 */
	...
	for (; bindtab->btype; bindtab++) {
		if (bindtab->btype == &(tatom)) {
			...
			Vreadtable->a.clb = currtab;	/* restore values for
							 * eval */
			...
			ibase->a.clb = inewint(10);	/* set back for next
							 * read */
...
)
{
	...
	if (Vpurcopylits->a.clb == nil) {
		*(intptr_t *) lc_org = (intptr_t) bind_lists;	/* point to current */
		bind_lists = (intptr_t *) lc_entries;	/* point to first item */
	}
	/*
	 * convert each literal into a lispval and put a pointer to it in the
	 * table.
	 */
 ...
 * rdstr
 * 
 * Read from a string.  Adapted from readlist (see lam6.c) by Jeff Dalton, AIAI,
 * University of Edinburgh
 * 
 * /\/ Neither this routine nor readlist handle unterminated strings corectly.
 * When reading something that begins with """ or "|", it will read forever
 * if no matching terminator is found.  The first sign of trouble is repeated
 * gc's for type vectori.
 * 
 */
...
rdstr(char *string)
{
	...
	case C_THROW:
		/*
		 * an error has occured and we are given a chance to unwind
		 * before the control goes higher lispretval contains the
		 * error descriptor in it's cdr
		 */
		fclose(p);	/* free port */
		errp = Popframe();
		lbot = np;
...
clinker(void)
{
	...
	/*
	 * Make link if (status translink) non-nil and function is compiled
	 * (bcd) and not foreign (string discipline).
	 */
