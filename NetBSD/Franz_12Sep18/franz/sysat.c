...
#else
__RCSID("$Id: sysat.c,v 1.8 2014/12/11 20:34:43 christos Exp $");
#endif
#endif
...
extern intptr_t *beginsweep;	/* place for garbage collector to begin
				 * sweeping */
extern int      page_limit;	/* begin warning messages about running out
				 * of space */
extern char     purepage[];	/* which pages should not be swept by gc */
extern int      ttsize;		/* need to know how much of pagetable to set
				 * to other */

static void     dofuns(void);
...
makevals(void)
{
	...
	/*
	 * allocate space for namestack and bindstack first then set up
	 * beginsweep variable so that the sweeper will ignore these `always
	 * in use' pages
	 */
	...
	/*
	 * since these dtpr pages will not be swept, we don't want them to
	 * show up in count of dtpr pages allocated or it will confuse
	 * gcafter when it tries to determine how much space is free
	 */
	dtpr_pages->i = 0;
	beginsweep = (intptr_t *) xsbrk(0);
...
#endif
	for (i = ATOX(beginsweep); i < ttsize; i++)
		(typetable + 1)[i] = OTHER;
	...
	Vpurcopylits->a.clb = tatom;	/* tells fasl to purcopy literals it
					 * reads */
	Vdisplacemacros = inewatom("displace-macros");
	Vdisplacemacros->a.clb = nil;	/* replace macros calls with their
					 * expanded forms */
	...
	Vlibdir->a.clb = inewatom(LISPDIR);
	...
	/*
	 * used as property list for nil, since nil will eventually be put at
	 * 0 (consequently in text and not writable)
	 */
	...
	stlist = nil;		/* initially nil */
	{
		...
		Isstatus(feature, dom = inewatom(LISP_DOMAIN));
	...
	noautot = inewatom("noautotrace");	/* option to inhibit
						 * auto-trace */
	Vgcprint = inewatom("$gcprint");	/* if t then pring gc
						 * messages */
	Vgcprint->a.clb = nil;

	(gcthresh = newint())->i = tgcthresh;
	gccall1 = newdot();
	gccall2 = newdot();	/* used to call gcafter */
...
/* */
lispval
mfun(char *string, lispval (*start)(void), lispval discip)
{
	lispval         v;
	v = inewatom(string);
	v->a.fnbnd = newfunct();
...
static struct ftab cfuns[] = {
	...
	{"top-level", Ntpl, &(nlambda)},	/* top level eval-print read
						 * loop  */
	...
	{"opval", Lopval, &(lambda)},	/* sets and retrieves system
					 * variables  */
	{"ncons", Lncons, &(lambda)},
	{"remob", Lforget, &(lambda)},	/* function to take atom out of hash
					 * table  */
	{"not", Lnull, &(lambda)},
	{"plus", Ladd, &(lambda)},
	{"add", Ladd, &(lambda)},
	...
	{"lessp", Llessp, &(lambda)},
	{"any-zerop", Lzerop, &(lambda)},	/* used when bignum arg
						 * possible */
	{"zerop", Lzerop, &(lambda)},
	{"minusp", Lnegp, &(lambda)},
	{"onep", Lonep, &(lambda)},
	...
	{"I-throw-err", Lctcherr, &(lambda)},	/* directly force a throw or
						 * error */
	...
	{"character-index", Lcharindex, &(lambda)},	/* index of char in
							 * string */
