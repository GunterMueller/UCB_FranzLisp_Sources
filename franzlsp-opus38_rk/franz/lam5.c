...
#else
__RCSID("$Id: lam5.c,v 1.7 2014/12/11 17:12:17 christos Exp $");
#endif
#endif
...
/*
 * =========================================== - -	explode functions:
 * aexplode , aexplodec, aexploden - The following function partially
 * implement the explode functions for atoms. -  The full explode functions
 * are written in lisp and call these for atom args. -
 * -===========================================
 */

#include "chars.h"
lispval
Lexpldx(int kind, int slashify)		/* kind = 0 => explode to characters
					 * = 1 => explode to fixnums
					 * (aexploden) slashify = 0 => do not
					 * quote bizarre characters = 1 =>
					 * quote bizarre characters */
{
	int             typ, i;
	char            ch, *strb, strbb[BUFSIZ];	/* temporary string
							 * buffer */
	lispval         last, handy;
	extern int      uctolc;
	char           *cp;
	Savestack(3);		/* kludge save mask */
	Keepxs();
	...
	*strbuf = 0;
	...
	case STRNG:
		...
		for (cp = (char *) handy; *cp; cp++) {
			if (slashify &&
			    (*cp == Xsdc || synclass(ctable[(unsigned char) *cp]) == CESC))
				*strb++ = Xesc;
			*strb++ = *cp;
		}
		if (slashify)
			...
			*out++ = *cp++;
			for (; *cp; cp++) {
				if (((ctable[(unsigned char) *cp] & QUTMASK) == QALWAYS) ||
				    (uctolc && isupper((unsigned char) *cp)))
					*out++ = Xesc;
				*out++ = *cp;
			}
			*out = 0;
	...
	case INT:
		strb = strbb;
		sprintf(strb, "%jd", (intmax_t)lbot->val->i);
		break;
	case DOUB:
		strb = strbb;
	...
	if (strb[0] != NULL_CHAR) {	/* if there is something to do */
		...
		for (i = 0; (ch = strb[i++]) != '\0';) {
			...
			case 0:
				strbuf[0] = hash = ch;	/* character explode */
...
Lchdir(void)
{
	...
	chkarg(1, "chdir");
	filenm = verify(lbot->val, "chdir - non symbol or string arg");
	if (chdir(filenm) >= 0)
		return (tatom);
	else
		return (nil);
...
/*
 * ========================================================== - -	ascii
 * - convert from number to ascii character - - form:(ascii number) - -	the
 * number is checked so that it is in the range 0-255 - then it is made a
...
Lfloat(void)
{
	...
	default:
		return errorh1(Vermisc, "Bad argument to float", nil, FALSE, 0, handy);
	}
}
double
Ifloat(lispval handy)
{
	double          sum = 0.0;
	int             count = 0;
...
urk:
	default:
		return error("First arg to divide neither a bignum nor int.", FALSE);
	}
	work = mynp->val;
	switch (TYPE(work)) {
	case INT:
...
LEmuldiv(void)
{
	...
	lispval         work, result;
	intptr_t        quo, rem;
	Savestack(3);		/* fix mask */

	/*
	 * (Emuldiv mul1 mult2 add quo) => temp = mul1 + mul2 + sext(add);
	 * result = (list temp/quo temp%quo); to mix C and lisp a bit
	 */
