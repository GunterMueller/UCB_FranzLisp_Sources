...
#else
__RCSID("$Id: ffasl.c,v 1.13 2014/12/25 23:57:55 christos Exp $");
#endif
#endif
...
/*
 * dispget - get discipline of function this is used to handle the tricky
 * defaulting of the discipline field of such functions as cfasl and
 * getaddress. dispget is given the value supplied by the caller, the error
 * message to print if something goes wrong, the default to use if nil was
 * supplied. the discipline can be an atom or string.  If an atom it is
 * supplied it must be lambda, nlambda or macro.  Otherwise the atoms pname
 * is used.
 */

lispval
dispget(lispval given, char *messg, lispval defult)
{
	int             typ;
...
Lcfasl(void)
{
	...
	switch (np - lbot) {
	case 3:
		protect(nil);	/* no discipline given */
	case 4:
		protect(nil);	/* no library given  */
	}
	chkarg(5, "cfasl");
	mlbot[0].val = verifyl(mlbot[0].val, "Incorrect .o file specification");
	mlbot[1].val = verifyl(mlbot[1].val, "Incorrect entry specification for cfasl");
	...
	if (work == nil)
		...
		largs = verify(work, "Bad loader flags");
	...
	snprintf(cbuf, sizeof(cbuf),
		"%s/nld -N -x -A %s -T %p %s -e %s -o %s %s -lc",
		Ilibdir(),
		ostabf,
		currend,
		mlbot[0].val->str,
	...
	work = newfunct();
	work->bcd.start = (lispval(*) (void))(intptr_t)header.a_entry;
	...
	/*** VMS cfasl code ***/
	{
		...
		int             cvt_unix_to_vms();	/* Convert UNIX to VMS
							 * filename */
		...
		if (largs == 0)
			largs = " ";
		snprintf(objfil, sizeof(objfil), "tmp:cfasl%d.tmp", pid);
		symfil[cvt_unix_to_vms(ostabf, symfil)] = 0;
		snprintf(cbuf,	sizeof(cbuf), /* Create link cmd. */
		...
		/*
	         ...
	         */
		snprintf(filename, sizeof(filename), "tmp:sym%d.stb", pid);
		unlink(filename);
		/*
	         * Rename the new symbol table so it is now the old symbol table
	         */
		snprintf(symfil, sizeof(symfil), "tmp:sym%d.new", pid);
		link(symfil, filename);
		unlink(symfil);
		snprintf(myname, sizeof(myname), "tmp:sym%d.stb", pid);
		stabf = myname;
		/*
...
/*
 * N.B. mytemp will return a pointer to static data that's overwritten on
 * each call, that classic bug.  Besides, the length might be too small, now
 * that we getenv("TMPDIR").  Lose, lose.  jd 17 oct 93
 */
...
{
	/*
	 * if(mypid==0) mypid = (getpid() & 0xffff); fails if you do a
	 * dumplisp after doing a cfasl
	 */
	snprintf(mybuff, sizeof(mybuff), "%s/Li%d.%d", tmpdir(),
	    (getpid() & 0xffff), seed++);
	return (mybuff);
}
...
verifyl(lispval in, char *error)
{
	for (EVER) {
		...
		case STRNG:
			return in;
		case ATOM:
			return (lispval)inewstr(in->a.pname);
		}
		in = errorh1(Vermisc, error, nil, TRUE, 0, in);
	}
}
...
/* extern	int fvirgin; */
/*
 * declared in ffasl.c tells if this is original lisp symbol table. if
 * fvirgin is 1 then we must copy the symbol table, else we can overwrite it,
 * since it is a temporary file which only one user could be using(was not
 * created as an original lisp or by a (dumplisp) or a (savelisp)).
 */

/* copy a block of data from one file to another of size size */
void
copyblock(FILE *f1, FILE *f2, intptr_t size)
{
	char            block[BUFSIZ];
 ...
 * 
 * removes the given entry points from the run time symbol table, so that later
 * cfasl'd files can have these label names.
 * 
 */
...
Lrmadd(void)
{
	...
	for (i = 0; i < numberofargs; i++, mlbot++) {
		mlbot->val = verifyl(mlbot->val, "Incorrect entry specification.");
		n = strlen((char *) mlbot->val);
		if (n > maxlen)
			maxlen = n;
	}
	/*
	 * Must not disturb object file if it an original file which other
	 * users can execute(signified by the variable fvirgin). so the
	 * entire symbol table is copied to a new file.
	 */
	if (fvirgin) {
		...
		if (N_BADMAG(buf)) {
			...
			fclose(f);
			fclose(fnew);
			errorh1(Vermisc, "Removeaddress: Bad file", nil, FALSE, 0, (lispval) inewstr(ostabf));
			{
				Freexs();
				return (nil);
			}
		...
		/*
		 * must be done before the structure buf is reassigned so
		 * that it will be accurate for the read file
		 */
	...
	if (N_BADMAG(buf)) {
		...
		while (n) {
			...
			/*
			 * compare block of symbols against list of entry
			 * point names given, if a match occurs, clear the
			 * N_EXT bit for that given symbol and signal a
			 * change.
			 */
			for (q = nlbuf; (m -= sizeof(struct nlist)) >= 0; q++) {
				...
				if (
				    (q->n_type & N_EXT) == 0
#if ! (os_unix_ts | os_unisoft | __APPLE__)
