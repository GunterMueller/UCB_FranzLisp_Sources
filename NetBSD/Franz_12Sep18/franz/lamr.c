...
#else
__RCSID("$Id: lamr.c,v 1.6 2014/12/11 17:12:17 christos Exp $");
#endif
#endif
...
Lforget(void)
{
	...
	if (hasht[hashv] == (struct atom *) lbot->val) {
		hasht[hashv] = lbot->val->a.hshlnk;
...
Lmfunction(void)
{
	...
	handy->bcd.start = lbot->val->f;	/* insert entry point */
...
Lreplace(void)
{
	...
	case DTPR:
		...
		return (a1);
	case BCD:
		a1->bcd.start = a2->bcd.start;
		a1->bcd.discipline = a2->bcd.discipline;
		return (a1);
	default:
		return errorh1(Vermisc, "Replace: cannot handle the type of this arg",
			       nil, FALSE, 0, a1);
	}
}
...
Lod(void)
{
	...
	for (i = 0; i < np->val->i; ++i)
		printf(copval(odform, CNIL)->a.pname, ((intptr_t *) (np[-2].val))[i]);
...
/*
 * this used to be Lwhat, but was changed to Lmaknum for maclisp compatiblity
 */
lispval
Lmaknum(void)
{
	chkarg(1, "maknum");
	return (inewint((intptr_t) (lbot->val)));
}
lispval
Lderef(void)
{
	...
	return (inewint(*(intptr_t *) (lbot->val->i)));
}
...
{
	chkarg(1, "lctrace");
	lctrace = (intptr_t) (lbot->val->a.clb);
...
Lsimpld(void)
{
	...
	for (pt = np->val; NOTNIL(pt); pt = pt->d.cdr)
		*(cpt++) = pt->d.car->i;
	*cpt = 0;
