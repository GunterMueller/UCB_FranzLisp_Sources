...
#else
__RCSID("$Id: lam4.c,v 1.6 2012/04/14 22:04:23 christos Exp $");
#endif
#endif
...
{
	lispval         work;
	struct argent  *result, *mynp, *oldnp;
	intptr_t        restype, prunep = FALSE, hi, lo = 0;
	...
	for (mynp = lbot; mynp < oldnp; mynp++) {
		...
		case INT:
			...
			case SDOT:
				...
				prunep = TRUE;
				/*
				 * In adding the fixnum to the sdot we may
				 * make it possible for the bignum to be
...
/*
 * exarith(a,b,c,lo,hi) int a,b,c; int *lo, *hi; Exact arithmetic. a,b and c
 * are 32 bit 2's complement integers calculates x=a*b+c to twice the
 * precision of an int. In the vax version, the 30 low bits only are returned
 * in *lo,and the next 32 bits of precision are returned in * hi. this works
 * since exarith is used either for calculating the sum of two 32 bit
 * numbers, (which is at most 33 bits), or multiplying a 30 bit number by a
 * 32 bit numbers, which has a maximum precision of 62 bits. If *phi is 0 or
 * -1 then x doesn't need any more than 31 bits plus sign to describe, so we
 * place the sign in the high two bits of *plo and return 0 from this
 * routine.  A non zero return indicates that x requires more than 31 bits to
 * describe.
 * 
...
Lsub(void)
{
	lispval         work;
	struct argent  *result, *mynp, *oldnp;
	intptr_t        prunep, restype, hi, lo = 0;
	struct sdot     dummybig;
	double          flacc;
	Savestack(4);
	...
	/*
	 * examine the first argument and perhaps set restype to the correct
	 * type.  If restype (result type) is INT, then the fixnum value is
	 * stored in lo.  Otherwise, if restype is SDOT or DOUB, then the
	 * value is stored in result->val.
	 */
	switch (TYPE(work)) {
	case INT:
		lo = work->i;
	...
	case SDOT:
		...
		work = adbig(work, (lispval) & dummybig);
		/*
		 * the resulting value may have been reduced from an sdot to
		 * a fixnum.  This should never happen though but if it does,
		 * we simplify things.
		 */
		restype = TYPE(work);
		if (restype == INT) {
	...
	/*
	 * now loop through the rest of the arguments subtracting them from
	 * the running result in result or lo
	 */
	for (; mynp < oldnp; mynp++) {
		work = mynp->val;
		switch (TYPE(work)) {
		case INT:
			...
			case SDOT:
				/*
				 * subtracting a fixnum from an bignum use
				 * the distructive multiply (by 1) and add
				 * the negative of the work value. The result
				 * will still be pointed to by result->val
				 */
				dmlad(result->val, 1L, -work->i);
			...
			case INT:
				/*
				 * subtracting a fixnum from a fixnum, the
				 * result could turn into a bignum
				 */
				if (exarith(lo, 1L, -work->i, &hi, &lo)) {
					work = result->val = newsdot();
					work->s.I = lo;
		...
		case SDOT:
			...
			case INT:
				/*
				 * subtracting a bignum from an integer first
				 * make a bignum of the integer and then fall
				 * into the next case
				 */
				dummybig.I = lo;
				dummybig.CDR = (lispval) 0;
			...
			case SDOT:
				 ...
				 * and should probably be written
				 * specifically for subtraction.
				 */
				work = subbig(result->val, work);
				pruneb(result->val);
		on1:
				/*
				 * check if the result has turned into a
				 * fixnum
				 */
				restype = TYPE(work);
				if (restype == INT) {
		...
		case DOUB:
			switch (restype) {
			case SDOT:	/* subtracting a flonum from a
					 * bignum. */
		...
		scrimp:	(result->val = newdoub())->r = flacc;
			...
			case INT:
				/*
				 * subtracting a flonum from an fixnum. The
				 * result will be an flonum.
				 */
				flacc = lo - work->r;
				goto scrimp;
			case DOUB:
...
Ltimes(void)
{
	lispval         work;
	struct argent  *result, *mynp, *oldnp;
	intptr_t        restype, prunep, hi, lo = 1;
	struct sdot     dummybig;
	double          flacc;
	Savestack(4);
...
Lquo(void)
{
	...
	if (oldnp == mynp)
		...
		case SDOT:
			/*
			 * we want to copy the sdot we are given as an
			 * argument since the bignum divide routine divbig
			 * expects an argument in canonical form.
			 */
			dummybig.I = 0;	/* create a zero sdot */
			work = adbig(work, (lispval) & dummybig);
			restype = TYPE(work);
			if (restype == INT) {	/* Either INT or SDOT */
				dummybig.I = work->i;	/* has turned into an
							 * fixnum */
				prunei(work);	/* return fixnum cell	     */
			} else {
				result->val = work;
	...
	/*
	 * now loop through the rest of the arguments dividing them into the
	 * running result in result or dummybig.I
	 */
	for (; mynp < oldnp; mynp++) {
		work = mynp->val;
		switch (TYPE(work)) {
		case INT:
			if (work->i == 0)
				kill(0, SIGFPE);
			switch (restype) {
			case SDOT:	/* there is no fast routine to
					 * destructively divide a bignum by
					 * an int, so do it the hard way. */
				dummybig.I = work->i;
		...
		on1:
				...
				if (restype == INT) {	/* Either INT or SDOT */
					dummybig.I = quotient->i;	/* has turned into an
									 * fixnum */
		...
		case SDOT:
			...
			case INT:
				/*
				 * Although it seems that dividing an int by
				 * a bignum can only lead to zero, it is
				 * concievable that the bignum is improperly
				 * boxed, i.e. actually an int.
				 */
		...
		case DOUB:
			switch (restype) {
			case SDOT:	/* Divide bignum by flonum converting
					 * to flonum May die due to overflow */
				flacc = Ifloat(result->val) / work->r;
				pruneb(result->val);
		scrimp:
				(result->val = newdoub())->r = flacc;
			...
			case INT:	/* dividing a flonum into a fixnum.
				...
				goto scrimp;
			case DOUB:	/* dividing a flonum into a flonum,
...
Lfexpt(void)
{
	...
	if ((TYPE(lbot[0].val) != INT) || (TYPE(lbot[1].val) != INT))
		errorh2(Vermisc, "^: non fixnum arguments", nil, 0, 0,
	...
	if (base == 0) {
		/*
		 * 0^0 == 1,  0 to any other power (even negative powers) is
		 * zero (according to Maclisp)
		 */
		if (exp == 0)
			return (inewint(1));
		else
	...
	/*
	 * compute exponentiation.  This should check for overflows, I
	 * suppose. --jkf
	 */
	res = 1;
	while (exp > 0) {
		if (exp & 1) {	/* odd, just multiply by one */
			res = res * base;
			exp--;
		} else {
			/* even, square base */
