...
#else
__RCSID("$Id: trace.c,v 1.4 2012/02/12 20:48:09 christos Exp $");
#endif
#endif
...
Levalhook(void)
{
	...
	case 3:
		...
		break;
	default:
		return argerr("evalhook");
	}

	/*
	 * Don't do this check any longer if (evalhsw == 0) error("evalhook
	 * called before doing sstatus-evalhook", TRUE); if (rsetsw == 0 ||
	 * rsetatom->a.clb == nil) error("evalhook called while not in *rset
	 * mode", TRUE);
	 */

	if (funhval != CNIL) {
		PUSHDOWN(funhatom, funhval);
	}
	PUSHDOWN(evalhatom, (lispval) (lbot + 1)->val);
	/*
	 * eval checks evalhcall to see if this is a LISP call to evalhook in
	 * which case it avoids call to evalhook function, but clobbers value
	 * to nil so recursive calls will check.
	 */
	evalhcallsw = TRUE;
	handy = eval(lbot->val);
	POP;
...
Lfunhook(void)
{
	...
	case 3:
		...
		break;
	default:
		return argerr("funcallhook");
	}

	/*
	 * Don't do this check any longer if (evalhsw == 0)
	 * error("funcallhook called before doing sstatus-evalhook", TRUE);
	 * if (rsetsw == 0 || rsetatom->a.clb == nil) error("funcallhook
	 * called while not in *rset mode", TRUE);
	 */
	...
	while (TYPE(handy) != DTPR)
		handy = errorh1(Vermisc, "funcallhook: first arg must be a list", nil, TRUE,
				0, handy);
	if (evalhval != CNIL) {
		PUSHDOWN(evalhatom, evalhval);
	}
	PUSHDOWN(funhatom, (lispval) (lbot + 1)->val);
	/*
	 * funcall checks funcallhcall to see if this is a LISP call to
	 * evalhook in which case it avoids call to evalhook function, but
	 * clobbers value to nil so recursive calls will check.
	 */
	funhcallsw = TRUE;
	/*
