...
#else
__RCSID("$Id: fex4.c,v 1.9 2014/12/11 17:12:17 christos Exp $");
#endif
#endif
...
/*
 * this is now a lambda function instead of a nlambda. the only reason that
 * it wasn't a lambda to begin with is that the person who wrote it didn't
 * know how to write a lexpr - jkf
 */
lispval
Lsyscall(void)
{
	...
	Restorestack();
	args[0] = acount - 1;
	return (inewint((intptr_t) callg((lispval(*) (void)) syscall, args)));
}

/*
 * eval-when: this has the form (eval-when <list> <form1> <form2> ...) where
 * the list may contain any combination of `eval', `load', `compile'. The
 * interpreter (us) looks for the atom `eval', if it is present we treat the
 * rest of the forms as a progn.
 */
...
/*
 * Status functions. These operate on the statuslist stlist which has the
 * form: ( status_elem_1 status_elem_2 status_elem_3 ...) where each status
 * element has the form: ( name readcode setcode .  readvalue) where name -
 * name of the status feature (the first arg to the status function).
 * readcode - fixnum which tells status how to read the value of this status
 * name.  The codes are #defined. setcode - fixnum which tells sstatus how to
 * set the value of this status name readvalue - the value of the status
 * feature is usually stored here.
 * 
 * Readcodes:
 * 
 * ST_READ - if no second arg, return readvalue. if the second arg is given, we
 * return t if it is eq to the readvalue. ST_FEATR - used in (status feature
 * xxx) where we test for xxx being in the status features list ST_SYNT -
 * used in (status syntax c) where we return c's syntax code ST_INTB - read
 * stattab entry ST_NFETR - used in (status nofeature xxx) where we test for
 * xxx not being in the status features list ST_DMPR - read the dumpmode
 * ST_UNDEF - return the undefined functions in the transfer table
 * 
 * Setcodes: ST_NO -  if not allowed to set this status through sstatus. ST_SET
 * - if the second arg is made the readvalue. ST_FEATW - for (sstatus feature
 * xxx), we add xxx to the (status features) list. ST_TOLC - if non nil, map
 * upper case chars in atoms to lc. ST_CORE - if non nil, have bus errors and
 * segmentation violations dump core, if nil have them produce a bad-mem err
 * msg ST_INTB - set stattab table entry ST_NFETW - use in (sstatus nofeature
 * xxx) where we wish to remove xxx from the status feature list. ST_DMPW -
 * set the dumpmode ST_BCDTR - (ifdef RSET) if non nil, creat trace stack
 * entries for calls from BCD functions to BCD functions ST_GCSTR - (ifdef
 * GCSTRINGS) garbage collect strings
 */

lispval
Nstatus(void)
{
	lispval         handy, curitm, valarg;
	int             indx;
	time_t          ctim;
	int             typ;
	char           *cp;
	...
	while (TYPE(handy) != DTPR)
		handy = error("status: bad arg list", TRUE);

	curitm = Istsrch(handy->d.car);	/* look for feature */

	if (curitm == nil)
		return (nil);	/* non existant */

	if (handy->d.cdr == nil)
		valarg = (lispval) CNIL;
	else
		valarg = handy->d.cdr->d.car;

	/*
	 * now do the processing with curitm pointing to the requested item
	 * in the status list
	 */
	...
	case ST_READ:
		...
		if (valarg != (lispval) CNIL)
			...
			return (curitm->d.cdr->d.cdr->d.cdr);
	...
	return error("Internal error in status: Couldn't figure out request", FALSE);
}
lispval
Nsstatus(void)
{
...
lispval
Isstatus(lispval curnam, lispval curval)
{
	lispval         curitm, head;
	...
	if (curitm == nil)
		curitm = Iaddstat(curnam, ST_READ, ST_SET, nil);
	...
	case ST_NFETW:		/* remove from features list */
		...
		for (head = curitm->d.cdr; head != nil; head = head->d.cdr) {
			if (head->d.car == curval)
				curitm->d.cdr = head->d.cdr;
			else
				curitm = head;
		}
		return (nil);
	...
	case ST_DMPW:
		if (TYPE(curval) != INT ||
		    (curval->i != 413 &&
		     curval->i != 407 &&
		     curval->i != 410))
			errorh1(Vermisc, "sstatus: bad dump mode:",
				nil, FALSE, 0, curval);
		dmpmode = curval->i;
		return (curval);
	...
	case ST_TRAN:
		if (curval != nil) {
			Strans = (lispval) TRUE;
			/*
			 * the atom `on' set to set up all table to their bcd
			 * fcn if possible
			 */
			if (curval == matom("on"))
				clrtt(1);
...
/*
 * Istsrch - utility routine to search the status list for the name given as
 * an argument.  If such an entry is not found, we return nil
 */
...
lispval
Iaddstat(lispval name, int readcode, int setcode, lispval valu)
{
