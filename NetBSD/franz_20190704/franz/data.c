...
#else
__RCSID("$Id: data.c,v 1.10 2014/12/11 18:40:29 christos Exp $");
#endif
#endif
...
#ifdef notdef
char            firstalloc[NBPG] = {'x'};	/* first thing allocated in
						 * file */
#endif
...
lispval         gctab[GCTABLEN] =	/* global constant table for
					 * interpreter */
...
int             rlevel;		/* used to indicate depth of recursion in
				 * reader.  No longer really necessary */
...
/*
 * default mode for dumplisp (note this is decimal not octal)
 */
...
intptr_t       *bind_lists = (intptr_t *)CNIL;	/* lisp data for compiled
						 * code */
...
struct types
                atom_str =
{
	(char *) CNIL, 0, ATOMSPP, ATOM, ATOMWPP,
...
doub_str =
{
	(char *) CNIL, 0, DOUBSPP, DOUB, DOUBWPP,
