...
#else
__RCSID("$Id: error.c,v 1.6 2015/02/05 02:19:53 christos Exp $");
#endif
#endif
...
static lispval  Ierrorh(lispval, char *, lispval, int, int);

/*
 * error this routine is always called on a non-fatal error.  The first argu-
 * ment is printed out.  The second a boolean flag indicating if the error
 * routine is permitted to return a pointer to a lisp value if the "cont"
 * command is executed.
 */

/*
 * error from lisp C code, this temporarily replaces the old error allowing
 * us to interface with the new errset scheme with minimum difficulty.  We
 * assume that an error which comes to this routine is of an "undefined error
 * type" ER%misc .  Soon all calls to this routine will be removed.
 * 
 */

lispval
error(char *mesg, int contvl)
{
 ...
 * 
 * call is errorh(type,message,valret,contuab) where type is an atom which
 * classifys the error, and whose clb, if not nil is the name of a function
 * to call to handle the error. message is a character string to print to
 * describe the error valret is the value to return to an errset if one is
 * found, and contuab is non nil if this error is continuable.
 */


/* VARARGS5 */
static          lispval
Ierrorh(lispval type, char *message, lispval valret, int contuab, int uniqid)
{
	struct frame   *curp, *uwpframe = (struct frame *) 0;
	lispval         handy;
	lispval        *work = IEargs;
	int             limit = IElimit;
	int             pass, curdepth;
	...
	if ((handy = type->a.clb) != nil) {	/* if there is an error
						 * handler */
		handy = calhan(limit, work, type, uniqid, contatm, message, handy);
		if (contuab && (TYPE(handy) == DTPR))
			return (handy->d.car);
...
ps2:
	...
	for (curp = errp; curp != (struct frame *) 0; curp = curp->olderrp) {
		if (curp->class == F_CATCH) {
			/*
			 * interesting catch tags are ER%unwind-protect,
			 * generated by unwind-protect and ER%all, generated
			 * by errset
			 */
			if ((pass == 1) && (curp->larg1 == Veruwpt)) {
				...
				/*
				 * have found an errset to jump to. If there
				 * is an errset handler, first call that.
				 */
	...
	/*
	 * at this point, print error message and break, just like the
	 * current error scheme
	 */
	printf("%s ", message);
	while (limit-- > 0) {
		printr(*work++, stdout);
	...
	/*
	 * If automatic-reset is set we will now jump to top level, calling
	 * the reset function if it exists, or using the c rest function if
	 * it does not
	 */
	...
	if (TYPE(Verdepth->a.clb) != INT) {
		...
		curdepth = 1 + Verdepth->a.clb->i;
	PUSHDOWN(Verdepth, inewint(curdepth));

	switch (retval) {
	case C_RET:		/* attempt to return from error */
		if (!contuab)
			error("Can't continue from this error",
			      FALSE);
		popnames(errp->svbnp);
		errp = Popframe();
		Restorestack();
...
lispval
errorh(lispval type, char *message, lispval valret, int contuab, int uniqid)
{
	IElimit = 0;
	return Ierrorh(type, message, valret, contuab, uniqid);
}

lispval
errorh1(lispval type, char *message, lispval valret, int contuab, int uniqid,
    lispval arg1)
{
	IElimit = 1;
	IEargs[0] = arg1;
...
lispval
errorh2(lispval type, char *message, lispval valret, int contuab, int uniqid,
    lispval arg1, lispval arg2)
{
	IElimit = 2;
	IEargs[0] = arg1;
	IEargs[1] = arg2;
...
lispval
calhan(int limit, lispval *work, lispval type, int uniqid, lispval contuab,
    char *message, lispval hdlr)
{
	lispval         handy;
	Savestack(4);
	lbot = np;
	protect(hdlr);	/* funcall the handler */
...
badmem(int n)
{
	...
	snprintf(errbuf, sizeof(errbuf),
	    "Attempt to allocate beyond static structures (%d).", n);
	error(errbuf, FALSE);
	/* NOT REACHED */
}
...
argerr(char *msg)
{
	return errorh1(Vermisc, "incorrect number of args to",
 ...
 */
void
wnaerr(lispval fcn, lispval wantargs)
{
	if (Vinterrfcn == nil) {
	...
	if (Vinterrfcn->a.fnbnd != nil) {
		protect(fcn);
		protect(inewint((long) wantargs / 1000));	/* min */
		protect(inewint(((long) wantargs % 1000) - 1));	/* max */
		Ifuncal(Vinterrfcn);
