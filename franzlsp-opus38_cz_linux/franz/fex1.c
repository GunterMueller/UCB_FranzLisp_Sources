...
#else
__RCSID("$Id: fex1.c,v 1.4 2012/02/12 20:08:38 christos Exp $");
#endif
#endif
...
Nprog(void)
{
	...
	case C_GO:		/*
		...
		/*
		 * label not found in this prog, must go up to higher prog
		 */
...
/*
 * (errset form [flag])  if present, flag determines if the error message
 * will be printed if an error reaches the errset. if no error occurs, errset
 * returns a list of one element, the value returned from form. if an error
 * occurs, nil is usually returned although it could be non nil if err threw
 * a non nil value
 */
...
Nerrset(void)
{
	...
	flag = lbot->val->d.cdr;
	if (TYPE(flag) == DTPR)
		flag = eval(flag->d.car);
	else
		flag = tatom;	/* if not present , assume t */
	protect(flag);
...
/*
 * this was changed from throw to *throw 21nov79 it is now a lambda and
 * really should be called Lthrow
 */
lispval
Nthrow(void)
{
...
Ndef(void)
{
	lispval         arglist, body, name, form;
