...
#else
__RCSID("$Id: io.c,v 1.7 2014/12/11 17:12:17 christos Exp $");
#endif
#endif
...
struct readtable {
	unsigned char   ctable[132];
		...
		/*
		 * ^@ nul	^A soh	^B stx	^C etx	^D eot	^E eng	^F
		 * ack	^G bel
		 */
		VERR, VERR, VERR, VERR, VERR, VERR, VERR, VERR,
		/*
		 * ^H bs	^I ht	^J nl	^K vt	^L np	^M cr	^N so
		 * ^O si
		 */
		VCHAR, VSEP, VSEP, VSEP, VSEP, VSEP, VERR, VERR,
		/*
		 * ^P dle	^Q dc1	^R dc2	^S dc3	^T dc4	^U nak	^V
		 * syn	^W etb
		 */
		VERR, VERR, VERR, VERR, VERR, VERR, VERR, VERR,
		/*
		 * ^X can	^Y em	^Z sub	^[ esc	^\ fs	^] gs	^^ rs
		 * ^_ us
		 */
...
readrx(int code)
{
	lispval         work;
	lispval        *current;
	struct argent  *result;
	int             inlbkt = FALSE;
	Savestack(4);		/* ???not necessary because np explicitly
				 * restored if changed */
...
top:
		...
		mcom:
			...
			case TPERD:
				...
				*current = work->d.car;
				 ...
				 * expression following the dot is terminated
				 * with a "]" and thus needs no closing
				 * lparens to follow
				 */
				if (rbktf && inlbkt)
					rbktf = FALSE;
	...
	case TPERD:
		...
		/*
		 * should never get here, we should have covered all cases
		 * above
		 */
	default:
		Restorestack();
...
macrox(void)
{
	...
	protect(nil);
	atomval = Lapply();
	chkrtab(Vreadtable->a.clb);	/* the macro could have changed the
					 * readtable */
...
void
imacrox(lispval current, int inlist)
{
	FILE           *svport;
	lispval         work;
	lispval         handy;
...
again:	cc = getc(useport);
	...
	*name = c;
	...
	switch (synclass(ctable[(unsigned char) c])) {
	...
	case synclass(VCHAR):
		if (uctolc && isupper((unsigned char) c))
	...
	case synclass(VPERD):
		marker = peekc(useport) & 0177;
		if (synclass(VNUM) != synclass(ctable[(unsigned char) marker])) {
			if (SEPMASK & ctable[(unsigned char) marker])
				return (TPERD);
			else {
				*name++ = c;	/* this period begins an atm */
				atomval = finatom(name);
				return (TSCA);
			}
		}
	...
	case synclass(VDQ):
		...
		while ((c = getc(useport)) != marker) {
			...
			if (synclass(VESC) == synclass(ctable[(unsigned char) c]))
				c = getc(useport) & 0177;
			push();
			if (feof(useport)) {
				clearerr(useport);
...
getnum(char *name)
{
	...
	if (VNUM == ctable[*(unsigned char *) (name - 1)])
		sawdigit = 1;
	while (VNUM == next()) {
...
backout:
	...
	/*
	 * at this point we have [0-9]*\. , which might be a decimal int or
	 * the leading part of a float
	 */
	if (next() != VNUM) {
		if (c == 'e' || c == 'E' || c == 'd' || c == 'D')
			goto expt;
			...
			/*
			 * Here we have 1.x where x is not number but is a
			 * separator Here we have decimal int. NOT FORTRAN!
			 */
			if (!saweof)
				ungetc((int) c, useport);
			return (calcnum(strbuf, name - 1, 10));
		} else
			goto last;	/* return a symbol */
	}
mantissa:
	do {
		push();
	...
	/*
	 * Here we have [0-9]*\.[0-9]* three possibilities: next character is
	 * e,E,d or D in which case we examine the exponent [then we are
	 * faced with a similar situation to this one: is the character after
	 * the exponent a separator or not] next character is a separator, in
	 * which case we have a number (without an exponent) next character
	 * is not a separator in which case we have an atom (whose prefix
	...
	if (stats & SEPMASK)
		goto verylast;	/* a real number */
	else
		goto last;	/* prefix makes it look like a number, but it
				 * isn't */
...
expt:
	...
	/*
	 * if a separator follows then we have a number, else just an atom
	 */
	if (stats & SEPMASK)
		goto verylast;

last:				/* get here when what looks like a number
				 * turns out to be an atom */
	if (!saweof)
		ungetc((int) c, useport);
	return (finatom(name));
...
verylast:
	...
	if (exploc != (char *) 0)
		*exploc = 'e';
	*name = 0;
	sscanf(strbuf, "%lf", &realno);
	(result = newdoub())->r = realno;
	return (result);
}

lispval
dopow(char *part2, int base)
{
	char           *name = part2;
	FILE           *useport = rdrport;
	int             power;
...
lispval
calcnum(char *strbf, char *name, int base)
{
	char           *p;
	lispval         result, temp;
	int             negflag = 0;
...
val_loop:
	if (!VALID(a)) {
		...
		if (debugmode != nil) {
			printf("<printr:bad lisp data: %p>\n", a);
			error("Bad lisp data encountered by printr", FALSE);
		} else {
			a = badst;
			printf("<printr:bad lisp data: %p>", a);
			return;
		}
	}
	switch (TYPE(a)) {
	...
	case INT:
		fprintf(useport, "%jd", (intmax_t)a->i);
		break;
...
veccommon:
		/*
		 * print out 'vector' or 'vectori' except in these
		 * circumstances: property is a symbol, in which case print
		 * the symbol's pname property is a list with a 'print'
		 * property, in which case it is funcalled to print the
		 * vector
		 */
	...
	case BCD:
		fprintf(useport, "#%lX-", (long) a->bcd.start);
		printr(a->bcd.discipline, useport);
		break;

	case OTHER:
		fprintf(useport, "#Other-%lX", (long) a);
		break;
...
out:
	...
	case ATOM:{
			...
			if (Idqc) {
				clean = first = *temp;
				first &= 0177;
				switch (QUTMASK & ctable[(unsigned char) first]) {
				case QWNFRST:
				case QALWAYS:
					clean = 0;
					break;
				...
				if (first == '-' || first == '+')
					temp++;
				if (synclass(ctable[(unsigned char) *temp]) == VNUM)
					clean = 0;
				while (clean && *temp) {
					if ((ctable[(unsigned char) *temp] & QUTMASK) == QALWAYS)
						clean = 0;
					else if (uctolc && (isupper((unsigned char) *temp)))
						clean = 0;
					temp++;
				}
				if (clean && !strflag)
					...
					for (temp = front; *temp; temp++) {
						if (*temp == Idqc
						    || (synclass(ctable[(unsigned char) *temp])) == CESC)
							putc(Xesc, useport);
						putc(*temp, useport);
					}
					putc(Idqc, useport);
				...
				for (; *cp; cp++) {
					if ((ctable[(unsigned char) *cp] & QUTMASK) == QALWAYS)
						putc(Xesc, useport);
					putc(*cp, useport);
				}
			}
...
/*
 * -- vectorpr (perhaps) print out vector specially this is called with a
 * vector whose property list begins with a list.  We search for the 'print'
 * property and if it exists, funcall the print function with two args: the
 * vector and the port. We return TRUE iff we funcalled the function, else we
 * return FALSE to have the standard printing done
 */

int
vectorpr(lispval vec, FILE *port)
{
	lispval         handy;
...
void
lfltpr(char *buf, double val) /* lisp floating point printer */
{
	char           *cp1;
	...
	/*
	 * if we are here, there was no dot, so the number was an integer.
	 * Furthermore, cp1 already points to the end of the string.
	 */
