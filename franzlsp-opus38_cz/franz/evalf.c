#include <sys/cdefs.h>
#ifndef lint
__RCSID("$Id: evalf.c,v 1.4 2012/02/12 20:48:09 christos Exp $");
#endif
#include "global.h"
#include "frame.h"
...
Levalf(void)
{
	...
	if (handy == nil) {	/* Arg of nil means start at the top */
		 ...
		 * read in, the Franz reader produces a bignum, thus giving
		 * some protection from being hacked.
		 */
	...
	handy->d.car = inewint((long) myfp);	/* The frame pointer as a
						 * lisp int */
	handy->d.cdr = newdot();
	handy = handy->d.cdr;
	if (evaltype)
		handy->d.car = myfp->larg1;	/* eval type - simply the arg
						 * to eval */
	else {			/* apply type ; must build argument list. The
				 * form will look like
				 * 
				 * (<function> (<evaled arg1> <evaled arg2>
				 * ....)) i.e. the function name followed by
				 * a list of evaluated args */
	...
	handy->d.car = inewint(myfp->svnp - orgnp);	/* index of np in
							 * namestack */
	handy->d.cdr = newdot();
	handy = handy->d.cdr;
	handy->d.car = inewint(myfp->svlbot - orgnp);	/* index of lbot in
							 * namestack */
	Restorestack();
	return (result);
}
...
struct frame *
searchforpdl(struct frame *myfp)
{
	/*
...
vfypdlp(struct frame *curfp)
{
	...
	for (myfp = errp; myfp != (struct frame *) 0; myfp = myfp->olderrp)
		if (myfp == curfp)
			return;
	errorh1(Vermisc, "Invalid pdl pointer given: ", nil, FALSE, 0, inewint((long) curfp));
}
