...
#else
__RCSID("$Id: fex2.c,v 1.5 2014/12/11 17:12:17 christos Exp $");
#endif
#endif
...
Ndo(void)
{
	...
	case ATOM:		/* This is old style maclisp do; atom is var,
				 * cadr(current) = init; caddr(current) =
				 * repeat etc. */
		if (varstuff == nil)
			goto newstyle;
		...
		*renewals = (current = current->d.cdr)->d.car;
		...
		case C_GO:	/*
			...
			/*
			 * label not found in this prog, must go up to higher
			 * prog
			 */
...
newstyle:
	case DTPR:		/* New style maclisp do; atom is list of
				 * things of the form (var init repeat)		 */
		count = 0;
		startnp = np;
		for (where = varstuff; where != nil; where = where->d.cdr) {
			/* do inits and count do vars. */
			/*
			 * requires "simultaneous" eval of all inits
			 */
		...
		for (index = 0; index < count; index++) {
			...
			if (handy == nil)
				handy = CNIL;	/* be sure not to rebind
						 * later */
			else
				handy = handy->d.car;
		...
		/*
		 * a do form with a test of nil just does the body once and
		 * returns nil
		 */
		if (handy == nil)
			repeatdo = 1;	/* just do it once */
		else
			repeatdo = -1;	/* do it forever   */
		...
		while (TRUE) {
			...
			case C_GO:	/*
				...
				/*
				 * label not found in this prog, must go up
				 * to higher prog
				 */
				Inonlocalgo(C_GO, lispretval, nil);
	...
	bodystart:
			...
			mybnp = savedbnp;
			for (index = 0; index < count; index++) {
				if (getem->val != CNIL)	/* if this atom has a
							 * repeat */
	...
	default:
		return error("do: neither list nor atom follows do", FALSE);
	}
}
...
Nprogv(void)
{
	...
	nameptr = oldbnp;
	/*
	 * simultaneous eval of all args
	 */
	for (; handy != nil; handy = handy->d.cdr) {
		(np++)->val = (handy->d.car);
		/*
		 * Note, each element should not be reevaluated like it was
		 * before.  - dhl
		 */
	...
	for (handy = lbot[1].val, vars = lbot + 3; handy != nil; handy = handy->d.cdr) {
		nameptr->atm = handy->d.car;
		++nameptr;	/* protect against interrupts while re-lambda
				 * binding */
		bnp = nameptr;
		nameptr[-1].atm = handy->d.car;
		nameptr[-1].val = handy->d.car->a.clb;
		if (vars < np)
