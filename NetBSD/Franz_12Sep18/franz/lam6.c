...
#else
__RCSID("$Id: lam6.c,v 1.5 2014/12/07 21:03:33 christos Exp $");
#endif
#endif
...
lispval
Lsetpli(void)
{				/* set the property list of the given atom to
				 * the given list */
	lispval         atm, vall;
...
Lsignal(void)
{
	lispval         handy, old, routine;
	int             i;

	switch (np - lbot) {

	case 1:
		routine = nil;	/* second arg defaults to nil */
		break;
...
Larg(void)
{
	...
	if (TYPE(lbot->val) != INT || (offset = lbot->val->i - 1) > count || offset < 0)
		return error("Out of bounds: arg to \"Arg\"", FALSE);
...
/*
 * (err-with-message message [value]) 'message' is the error message to
 * print. 'value' is the value to return from the errset (if present). it
 * defaults to nil. The message may not be printed if there is an (errset ...
 * nil) pending.
 */
...
Lerr(void)
{
	...
	switch (np - lbot) {
	case 2:
		valret = lbot[1].val;	/* return non nil */
	case 1:
		mesg = verify(lbot[0].val,
		    "err-with-message: non atom or string arg");
		break;
	default:
		return argerr("err-with-message");
	}
...
Ltyi(void)
{
	...
	if (val == EOF) {
		clearerr(port);
		if (sigintcnt > 0)
			sigcall(SIGINT);	/* eof might mean int */
		if (eofvalgiven)
			return (eofval);
		else
			return (inewint(-1));
...
/*
 * Untyi (added by DNC Feb. '80) - (untyi number port) puts the character
 * with ascii code number in the front of the input buffer of port.  Note
 * that this buffer is limited to 1 character.  That buffer is also written
 * by tyipeek, so a peek followed by an untyi will result in the loss of the
 * peeked char.
 */
...
Imkrtab(int current)
{
	...
	if ((cycle++) % 3 == 0) {
		...
		/*
		 * need to protect partially allocated read tables from
		 * garbage collection.
		 */
	}
	handy = newarray();
...
/*
 * makereadtable - arg : t or nil returns a readtable, t means return a copy
 * of the initial readtable
 * 
...
Lcpy1(void)
{
	lispval         handy = lbot->val, result = handy;
...
/*
 * copyint* . This returns a copy of its integer argument.  The copy will be
 * a fresh integer cell, and will not point into the read only small integer
 * table.
 */
lispval
Lcopyint(void)
{
	lispval         handy = lbot->val;
	lispval         ret;
